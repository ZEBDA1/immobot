...
def detect_scam(l: ScrapedListing) -> ScamResult:
    ...
    for kw in SUSPICIOUS_KEYWORDS:
        if kw in text:
            return ScamResult(is_scam=True, reason=f"Mot-clé suspect: {kw}")
    if l.price and l.surface_m2 and l.surface_m2 > 0:
        ppm2 = l.price / l.surface_m2
        if ppm2 < 1000:  # unrealistically low in many FR markets
            return ScamResult(is_scam=True, reason="Prix au m² anormalement bas")
    if l.price and l.price < 200:  # too low total price
